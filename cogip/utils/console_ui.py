...
class ConsoleUI(Console):
    ...
    async def get_integer(self, prompt: str, *, default: int | None = None) -> int:
        ...
        if default is not None:
            kwargs["default"] = default
        value = await loop.run_in_executor(None, lambda: IntPrompt.ask(f"[prompt]{prompt}[/]", **kwargs))
    ...
    async def get_float(self, prompt: str, *, default: float | None = None) -> float:
        ...
        if default is not None:
            kwargs["default"] = default
        return await loop.run_in_executor(None, lambda: FloatPrompt.ask(f"[prompt]{prompt}[/]", **kwargs))
    ...
    async def confirm(self, message: str, *, default: bool = True) -> bool:
        ...
        return await loop.run_in_executor(
            None, lambda: Confirm.ask(f"[header]{message}[/]", default=default, console=self)
        )
    ...
    async def wait_for_enter(self, message: str) -> None:
        ...
        """
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(None, lambda: self.input(f"[prompt]{message}[/] [muted][[Enter]][/] "))
