...
from cogip.tools.firmware_telemetry.graph.models import TelemetryGraphConfig
...
class PidCalibration:
    ...
    def __init__(
        ...
        # Timing
        self.sleep_between_motion_s: float = 0.5
    ...
    async def _select_pid_type(self) -> None:
        ...
        # Let user override motion test parameters
        await self._ask_motion_parameters()

    async def _ask_motion_parameters(self) -> None:
        """Prompt user for motion test parameters, overwriting PidType defaults."""
        self.console.show_rule("Motion Test Parameters")
        if self.pid_type.command_kind == CommandKind.POSE:
            unit = "mm" if self.pid_type.motion_kind == MotionKind.LINEAR else "deg"
            self.pid_type.default_distance = await self.console.get_float(
                f"Test distance ({unit})", default=float(self.pid_type.default_distance)
            )
        else:
            unit = "mm/s" if self.pid_type.motion_kind == MotionKind.LINEAR else "deg/s"
            self.pid_type.default_speed = await self.console.get_float(
                f"Test speed ({unit})", default=float(self.pid_type.default_speed)
            )
            self.pid_type.default_duration_ms = await self.console.get_integer(
                "Test duration (ms)", default=int(self.pid_type.default_duration_ms)
            )
        ...
        match (command, kind):
            case (CommandKind.POSE, MotionKind.LINEAR):
                ...
                if not await self.firmware.goto(dist, 0, 0):
                    ...
                    await self.firmware.reset()
                await asyncio.sleep(self.sleep_between_motion_s)
            ...
            case (CommandKind.SPEED, MotionKind.LINEAR):
                ...
                await asyncio.sleep(duration_s + self.sleep_between_motion_s)
    ...
    async def _pid_calibration_loop(self) -> None:
        ...
        while True:
            ...
            # Give enought time for last telemetry data
    ...
    async def _run_calibration(self) -> None:
        ...
        while True:
            ...
            self.console.show_info(f"Setting controller to {PB_ControllerEnum.Name(controller)}...")
            await self.firmware.set_controller(controller)
            self.console.show_success(f"Controller set to {PB_ControllerEnum.Name(controller)}")
