...
from cogip.tools.planner.cameras import get_crates_position, get_robot_position
...
class TestCaptureX1CameraStrategy(Strategy):
    def __init__(self, planner: "Planner"):
        super().__init__(planner)
        self.append(CaptureCratesCameraAction(planner, self, CollectionAreaID.LocalBottomSide, 2_000_000.0))
...
class TestCaptureX2CameraStrategy(Strategy):
    def __init__(self, planner: "Planner"):
        super().__init__(planner)

        self.append(CaptureCratesCameraAction(planner, self, CollectionAreaID.LocalBottomSide, 2_000_000.0))
        self.append(CaptureCratesCameraAction(planner, self, CollectionAreaID.LocalBottom, 1_900_000.0))
...
class CaptureAndDropCratesAction(CaptureCratesAction):
    async def after_capture(self):
        await super().after_capture()
    ...
    async def after_action(self):
        ...
        await crates_utils.init_grips(self.planner, self.side)


class TestCaptureAndDropStrategy(Strategy):
    def __init__(self, planner: "Planner"):
        super().__init__(planner)
...
class TestCaptureAndDropCameraStrategy(Strategy):
    def __init__(self, planner: "Planner"):
        super().__init__(planner)
        self.append(CaptureAndDropCratesCameraAction(planner, self, CollectionAreaID.LocalBottomSide, 2_000_000.0))
