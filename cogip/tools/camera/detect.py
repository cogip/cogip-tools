...
def get_camera_position_on_table(
    ...
    # Camera in Table frame (relative to Marker origin)
    R_tc = R_tm @ R_mc
    T_tc = R_tm @ T_mc

    # Camera in Table frame (absolute position)
    T_table = T_tc + table_markers_tvecs[marker_id]
...
def get_camera_position_in_robot(
    ...
    T_ct = table_camera_tvec

    # Camera in Robot frame (Transformation T_cr)
    # Position: T_cr = R_rt.T * (T_ct - T_rt)
    # Rotation: R_cr = R_rt.T * R_ct
    T_cr = R_rt.T @ (T_ct - T_rt)
    R_cr = R_rt.T @ R_ct
