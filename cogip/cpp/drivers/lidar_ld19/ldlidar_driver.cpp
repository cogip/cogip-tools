...
void LDLidarDriver::setLaserScanData(Points2D &src) {
    ...
    for (size_t angle = 0; angle < NUM_ANGLES; angle++) {
        ...
        if (size > 0) {
            lidar_points_[angle][0] = std::accumulate(distances.begin(), distances.end(), 0) / size;
            lidar_points_[angle][1] = std::accumulate(intensities.begin(), intensities.end(), 0) / size;
        }
        else {
            lidar_points_[angle][0] = max_distance;  // Mark as invalid
            lidar_points_[angle][1] = 0;  // Mark as invalid
        }
    }
}
