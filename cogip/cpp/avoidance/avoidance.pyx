...
cdef class CppPolygon:
    ...
    def __getitem__(self, int index) -> Coords:
        """Return the point at the given index."""
        cdef Coords p
        cdef CppCoords cpp_coords
        if index < 0 or index >= self.c_polygon.size():
            raise IndexError("Index out of range")
        p = self.c_polygon.at(index)
        cpp_coords = CppCoords(p.x(), p.y())
        return cpp_coords
...
cdef class CppObstacle:
    ...
    def is_edge_valid(self, double x1, double y1, double x2, double y2):
        return not self.c_obstacle.is_segment_crossing(Coords(x1, y1), Coords(x2, y2))
